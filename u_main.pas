...
uses
  ...
  DBCtrls, ShellAPI, Spin, DB, AdvTouchKeyboard, StrUtils, AdvSmoothTouchKeyBoard, Grids, BaseGrid, AdvGrid, VidGrab, SkinExCtrls,
  RzStatus, ActnList;
...
type
  TForm1 = class(TForm)
    ...
    procedure AddHourClick(Sender: TObject);
  private
...
uses u_dm, u_effect, Registry, u_layar, u_saran, u_save_song,
  u_open_song, u_pesan, u_request, u_record, uPopup, uPassword, uFood, u_Help,
  uAddHour;
...
begin
  ...
  begin
    dm.alert('Teknisi Mode Sudah Running', dm.spSkinMessage1);
  end;
end;
...
begin
  ...
  begin
    ...
    if Depan.Tag = 1 then
      dm.ListSong.SQL.Add('select judul, artis, direktori, namaFile, volume from song_list where bahasa='+QuotedStr(Edit1.Text)+
  ...
  begin
    ...
    dm.ListSong.SQL.Clear;
    if Depan.Tag = 1 then
      dm.ListSong.SQL.Add('select judul, artis, direktori, namaFile, volume from song_list '+
...
begin
  ...
  begin
    ...
    LinkControlToArea('AddHour', AddHour);
    LinkControlToArea('help', help1);
    LinkControlToArea('JamSisa', panel1);
  end;
end;
...
// xVideo_LoadPlugin3('xVideo_AAC.dll',0);
  ...
  xVideo_LoadPlugin2(PChar('C:\program files\K-Lite Codec Pack\ffdshow\ffmpeg.dll'),
...
// ComboBox1Click(Sender);
  xVideo_SetConfig(xVideo_VideoRenderer,xVideo_VMR9WindowsLess);
...
procedure TForm1.AddHourClick(Sender: TObject);
begin
  frmAddHour.ShowModal;
end;
...
begin
  ...
  if ASisa >= 0 then Panel1.Caption := SecToTime(ASisa);

  if (Trunc(ASisa) <=15) and (Trunc(ASisa) >= 0)//  123error keneh
  then begin
    autoCek := false;
    Form3.marqueeList.Caption := 'Info : '+IntToStr(ASisa)+' menit lagi waktu anda habis';
//    Form3.marqueeList.Color := clYellow;
  end;
  if (Trunc(ASisa) <= 0) then
  begin
    autoCek := false;
    Form3.marqueeList.Caption := '';
//    Form3.marqueeList.Font.Color := clRed;
  ...
  dm.openSQL(dm.qexec2,'select hist from room_hist where hist <> ''0'' and id_room='+QuotedStr(DM.RoomID));
  if dm.qexec2.RecordCount > 0 then begin
    Timer1.Enabled := false;
    if DM.qexec2.Fields[0].AsString = '1' then
     Application.MessageBox('Permintaan penambahan jam anda ditolak'+#13+
     'dengan alasan JAM OPERASIONAL SUDAH HABIS','Add Hour', 0) else
     Application.MessageBox('Permintaan penambahan jam anda ditolak'+#13+
     'dengan alasan ADA WAITING LIST','Add Hour', 0);
    dm.execSQL(dm.qexec2,'delete from room_hist where id_room='+QuotedStr(DM.RoomID));
    Timer1.Enabled := true;
  end;
...
begin
    if DM.Pesan = true then begin
      ...
      if (dm.qexec.RecordCount > 0) and
      (dm.qexec.Fields[0].AsInteger = 0) and
      (not DM.ShowMsg) then begin
          DM.Pesan := false;
          f_pesan.ShowModal;
      end;
    end;
